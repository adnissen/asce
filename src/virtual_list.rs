//! Virtual List for rendering a large number of differently sized rows/columns.
//!
...
//!
//! Unlike the `uniform_list`, each item can have different size.
//!
                    ...
                    |state: Option<ItemSizeLayout>, _window| {
                        ...
                        if state.items_sizes != self.item_sizes {
                            ...
                            state.content_size = if matches!(self.axis, Axis::Horizontal) {
                                Size {
                                    width: state.sizes.iter().fold(px(0.0), |acc, &size| acc + size),
                                    height: longest_item_size.height,
                                }
                            } else {
                                Size {
                                    width: longest_item_size.width,
                                    height: state.sizes.iter().fold(px(0.0), |acc, &size| acc + size),
                                }
                            };
                        }
